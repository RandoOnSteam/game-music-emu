...
// Output
	...
	int avail() const { return avail_( (int)(write_pos - &buf [width_ * stereo]) ); }
...
class Fir_Resampler : public Fir_Resampler_ {
	BOOST_STATIC_ASSERT( width >= 4 && width % 2 == 0, "FIR width must be even and have 4 or more points"  );
...
int Fir_Resampler<width>::read( sample_t* out_begin, int32_t count )
{
	...
	write_pos = &buf [left];
	blarg_memmove( buf.begin(), in, left * sizeof *in );

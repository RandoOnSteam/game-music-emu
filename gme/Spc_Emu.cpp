...
Spc_Emu::Spc_Emu()
{
	...
	set_voice_keycodes( apu.voice_keycodes() );

	set_voice_states( apu.voice_states() );

	set_voice_programs( apu.voice_programs() );
}
		...
		switch ( id )
		{
			...
			*/
			...
			case 0x13:
				...
				blarg_memcpy( &copyright [year_len], in, copyright_len );
				break;
...
{
	BOOST_STATIC_ASSERT( offsetof (header_t,unused2 [46]) == header_size, "SPC Header layout incorrect!" );
	file_data = in;
	file_size = size;

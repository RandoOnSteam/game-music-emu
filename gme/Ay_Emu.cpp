...
{
	ptrdiff_t pos = ptr - (byte const*) file.header;
	ptrdiff_t file_size = file.end - (byte const*) file.header;
...
{
	BOOST_STATIC_ASSERT( offsetof (header_t,track_info [2]) == header_size, "AY Header layout incorrect!" );
...
blargg_err_t Ay_Emu::start_track_( int track )
{
	...
	blarg_memset( mem.ram + 0x0000, 0xC9, 0x100 ); // fill RST vectors with RET
	blarg_memset( mem.ram + 0x0100, 0xFF, 0x4000 - 0x100 );
	blarg_memset( mem.ram + ram_start, 0x00, sizeof mem.ram - ram_start );
	blarg_memset( mem.padding1, 0xFF, sizeof mem.padding1 );
	blarg_memset( mem.ram + 0x10000, 0xFF, sizeof mem.ram - 0x10000 );
	...
	{
		blocks += 2;
		size_t len = get_be16( blocks ); blocks += 2;
		if ( addr + len > 0x10000 )
		{
	...
	blarg_memcpy( mem.ram, passive, sizeof passive );
	unsigned play_addr = get_be16( more_data + 4 );
	//debug_printf( "Play: $%04X\n", play_addr );
	if ( play_addr )
	{
		blarg_memcpy( mem.ram, active, sizeof active );
		mem.ram [ 9] = play_addr;

...
bool Gb_Cpu::run( int32_t cycle_count )
{
	...
	state_t s;
	this->state = &s;
	blarg_memcpy( &s, &this->state_, sizeof s );
	...
	BOOST_STATIC_ASSERT( sizeof rg == 8 && sizeof rp == 8, "Unhandled word size" );
...
stop:
	...
	this->state = &state_;
	blarg_memcpy( &this->state_, &s, sizeof this->state_ );

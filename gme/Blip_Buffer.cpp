...
Silent_Blip_Buffer::Silent_Blip_Buffer()
{
	...
	blarg_memset( buf, 0, sizeof buf ); // in case machine takes exception for signed overflow
}
...
void Blip_Buffer::clear( int entire_buffer )
{
	...
	if ( buffer_ )
	{
		...
		blarg_memset( buffer_, 0, (count + blip_buffer_extra_) * sizeof (buf_t_) );
	}
}
...
void Blip_Buffer::remove_samples( long count )
{
	if ( count )
	{
		...
		blarg_memmove( buffer_, buffer_ + count, remain * sizeof *buffer_ );
		blarg_memset( buffer_ + remain, 0, count * sizeof *buffer_ );
	}
}
